...
Matrix::Matrix(std::initializer_list<std::initializer_list<double>> l)
  : Matrix(std::vector<std::vector<double>>(l.begin(), l.end())) // contructor taking pointer to start and end of data
{
}
...
std::vector<double> Matrix::get_col(size_t i) const
{
...
{
  if (m_columns >= col && m_rows >= row)
    return m_data[row][col];
  throw std::invalid_argument("Specified index not in Matrix!");
}